...
<div id="entry">        
    <form name="frm_aturadmin" onsubmit="return validasiIsi();" method="post" action="" enctype=multipart/form-data>
        <?php
            ...
            $jabatan            = validTeks(isset($_GET['jabatan'])?$_GET['jabatan']:NULL);
        ...
        <?php
            ...
            if (isset($BtnSimpan)) {
                ...
                $tmt_pangkat        = validTeks(trim($_POST['ThnJabatan'])."-".trim($_POST['BlnJabatan'])."-".trim($_POST['TglJabatan']));
                $tmt_pangkat_yad    = validTeks(trim($_POST['ThnJabatanYad'])."-".trim($_POST['BlnJabatanYad'])."-".trim($_POST['TglJabatanYad']));
                ...
                $dokumen            = validTeks(str_replace(" ","_","pages/riwayatpangkat/berkas/".$_FILES['dokumen']['name']));
                                ...
                                if(($_FILES['dokumen']['type'] == 'image/jpeg')||($_FILES['dokumen']['type'] == 'image/jpg')){
                                    if((@mime_content_type($_FILES['dokumen']['tmp_name'])== 'image/jpeg')||(@mime_content_type($_FILES['dokumen']['tmp_name'])== 'image/jpg')){
                                        try {
                                            ...
                                            if($e->getCode()==1062){
                                                echo "<b style='color:red'>Data riwayat jabatan sudah ada..!!!</b>";
                                            }else{
            ...
            $_sql      = "SELECT riwayat_jabatan.jabatan,riwayat_jabatan.tmt_pangkat,riwayat_jabatan.tmt_pangkat_yad,riwayat_jabatan.pejabat_penetap,riwayat_jabatan.nomor_sk,riwayat_jabatan.tgl_sk,riwayat_jabatan.dasar_peraturan,riwayat_jabatan.masa_kerja,riwayat_jabatan.bln_kerja,riwayat_jabatan.berkas from riwayat_jabatan where riwayat_jabatan.id='$id' ORDER BY riwayat_jabatan.jabatan ASC ";
            $hasil     = bukaquery($_sql);
                ...
                while($baris = mysqli_fetch_array($hasil)) {  
                    ...
                    echo "<tr class='isi'>
                        ...
                        echo "</center>
                            ...
                            <td>$baris[6]</td>
                            <td align='center'>$baris[7] Tahun $baris[8] Bulan</td>
                         </tr>";
                    echo "<tr class='isi'>
                            <td width='70'></td>
    ...
    <?php
        if ($action=="HAPUS") {
            try {
                unlink($_GET['berkas']);
                Hapus(" riwayat_jabatan "," id ='".validTeks($_GET['id'])."' and jabatan ='".validTeks($_GET['jabatan'])."'","?act=InputRiwayatPangkat&action=TAMBAH&id=$id");
            } catch(mysqli_sql_exception $e) {
