<?php
    ...
    $_sqllibur = "select set_hari_libur.tanggal,set_hari_libur.ktg from set_hari_libur 
                  where set_hari_libur.tanggal like '%".$tahun."-".$bulan."%' ORDER BY set_hari_libur.tanggal";
        ...
        if (isset($cari["usere"])) {
            if(($cari["usere"]==USERHYBRIDWEB)&&($cari["passwordte"]==PASHYBRIDWEB)){
                ...
                $_sql = "select pegawai.id,pegawai.nik,pegawai.nama,pegawai.jbtn,pegawai.pendidikan,pegawai.mulai_kerja,pegawai.wajibmasuk,departemen.nama as departemen,
                        ...
                        DATE_FORMAT(pegawai.mulai_kerja, '%Y%m'))/12), ' Tahun ',MOD(PERIOD_DIFF(DATE_FORMAT('$tahun-$bulan-$hari', '%Y%m'), DATE_FORMAT(pegawai.mulai_kerja, '%Y%m')),12), ' Bulan ') as lama,
                        pendidikan.indek as index_pendidikan,(To_days('$tahun-$bulan-$hari')-to_days(pegawai.mulai_kerja))/365 as masker,stts_kerja.indek as index_status,
                        pegawai.indek as index_struktural,pegawai.pengurang,pegawai.mulai_kontrak,CONCAT(FLOOR(PERIOD_DIFF(DATE_FORMAT('$tahun-$bulan-$hari', '%Y%m'),DATE_FORMAT(pegawai.mulai_kontrak, '%Y%m'))/12), ' Tahun ',MOD(PERIOD_DIFF(DATE_FORMAT('$tahun-$bulan-$hari', '%Y%m'), DATE_FORMAT(pegawai.mulai_kontrak, '%Y%m')),12), ' Bulan ') as lamakontrak,
                        (To_days('$tahun-$bulan-$hari')-to_days(pegawai.mulai_kontrak))/365 as maskon, pegawai.cuti_diambil,pegawai.dankes
                ...
                while($baris = mysqli_fetch_array($hasilcari)) {
                    ...
                    if(isset($baris["wajibmasuk"])==-1){
                        ...
                        $jmlmsk=getOne("select (if(jadwal_pegawai.h1='',0,1)+if(jadwal_pegawai.h2='',0,1)+if(jadwal_pegawai.h3='',0,1)+if(jadwal_pegawai.h4='',0,1)+if(jadwal_pegawai.h5='',0,1)+"
                                       ."if(jadwal_pegawai.h6='',0,1)+if(jadwal_pegawai.h7='',0,1)+if(jadwal_pegawai.h8='',0,1)+if(jadwal_pegawai.h9='',0,1)+if(jadwal_pegawai.h10='',0,1)+"
                                       ."if(jadwal_pegawai.h11='',0,1)+if(jadwal_pegawai.h12='',0,1)+if(jadwal_pegawai.h13='',0,1)+if(jadwal_pegawai.h14='',0,1)+if(jadwal_pegawai.h15='',0,1)+"
                                       ."if(jadwal_pegawai.h16='',0,1)+if(jadwal_pegawai.h17='',0,1)+if(jadwal_pegawai.h18='',0,1)+if(jadwal_pegawai.h19='',0,1)+if(jadwal_pegawai.h20='',0,1)+"
                                       ."if(jadwal_pegawai.h21='',0,1)+if(jadwal_pegawai.h22='',0,1)+if(jadwal_pegawai.h23='',0,1)+if(jadwal_pegawai.h24='',0,1)+if(jadwal_pegawai.h25='',0,1)+"
                                       ."if(jadwal_pegawai.h26='',0,1)+if(jadwal_pegawai.h27='',0,1)+if(jadwal_pegawai.h28='',0,1)+if(jadwal_pegawai.h29='',0,1)+if(jadwal_pegawai.h30='',0,1)+"
                                       ."if(jadwal_pegawai.h31='',0,1)) from jadwal_pegawai where jadwal_pegawai.id='$baris[0]' and jadwal_pegawai.tahun='$tahun' and jadwal_pegawai.bulan='$bulan'");
                    ...
                    $_sql3    = "SELECT sum(jgmlm.jml) from jgmlm where jgmlm.id='$baris[0]' and jgmlm.tgl like '%".$tahun."-".$bulan."%' group by jgmlm.id";
                    $hasil3   = bukaquery($_sql3);
                    $baris3   = mysqli_fetch_row($hasil3);
                    $jgmlm    = isset($baris3[0])?$baris3[0]:0;
                    $sisamlm  = (isset($baris3[0])?$baris3[0]:0)-0;

                    $_sql4    = "SELECT sum(ketidakhadiran.jml) from ketidakhadiran where ketidakhadiran.id='$baris[0]' and ketidakhadiran.tgl like '%".$tahun."-".$bulan."%' and ketidakhadiran.jns='A' group by ketidakhadiran.id";
                    $hasil4   = bukaquery($_sql4);
                    $baris4   = mysqli_fetch_row($hasil4);
                    $ttla     = isset($baris4[0])?$baris4[0]:0;

                    $_sql5    = "SELECT sum(ketidakhadiran.jml) from ketidakhadiran where ketidakhadiran.id='$baris[0]' and ketidakhadiran.tgl like '%".$tahun."-".$bulan."%' and ketidakhadiran.jns='S' group by ketidakhadiran.id";
                    $hasil5   = bukaquery($_sql5);
                    $baris5   = mysqli_fetch_row($hasil5);
                    $ttls     = isset($baris5[0])?$baris5[0]:0;

                    $_sql6    = "SELECT sum(ketidakhadiran.jml) from ketidakhadiran where ketidakhadiran.id='$baris[0]' and ketidakhadiran.tgl like '%".$tahun."-".$bulan."%' and ketidakhadiran.jns='C' group by ketidakhadiran.id";
                    $hasil6   = bukaquery($_sql6);
                    $baris6   = mysqli_fetch_row($hasil6);
                    $ttlc     = (isset($baris6[0])?$baris6[0]:0)+getOne("select sum(pengajuan_cuti.jumlah) from pengajuan_cuti where pengajuan_cuti.tanggal_awal like '%".$tahun."-".$bulan."%' and pengajuan_cuti.status='Disetujui' and pengajuan_cuti.nik='".$baris["nik"]."'");

                    $_sql7    = "SELECT sum(ketidakhadiran.jml) from ketidakhadiran where ketidakhadiran.id='$baris[0]' and ketidakhadiran.tgl like '%".$tahun."-".$bulan."%' and ketidakhadiran.jns='I' group by ketidakhadiran.id";
                    $hasil7   = bukaquery($_sql7);
                    $baris7   = mysqli_fetch_row($hasil7);
                    $ttli     = isset($baris7[0])?$baris7[0]:0;

                    $_sql8    = "SELECT sum(tambahjaga.jml) from tambahjaga where tambahjaga.id='$baris[0]' and tambahjaga.tgl like '%".$tahun."-".$bulan."%' group by tambahjaga.id";
                ...
                $hasilindex=bukaquery("select indekref.kdindex,indekref.n,indekref.ttl from indekref");
                while($barisindex = mysqli_fetch_array($hasilindex)) {
                    $_sql22  ="SELECT ($barisindex[1]/sum(indekref.n))*100 from indekref where indekref.kdindex='$barisindex[0]'";
                ...
                if(mysqli_num_rows($hasil)!=0) {
                    echo "<table width='100%' border='0' align='center' cellpadding='0' cellspacing='0' class='tbl_form'>
                            <caption><h3><font color='999999'>Laporan Cash Gaji Karyawan Tahun ".$tahun." Bulan ".$bulan."</font></h3></caption>
                            <tr class='head'>
                                 ...
                                 <td width='10%'><div align='center'>BPD</div></td>
                                 <td width='30%'><div align='center'><font color='green'>Total Gaji Diterima</font></div></td>
                            </tr>";
                            while($baris = mysqli_fetch_array($hasil)) {
                                $_sqlgp    = "SELECT pendidikan.gapok1,pendidikan.kenaikan,pendidikan.maksimal from pendidikan where pendidikan.tingkat='".$baris["pendidikan"]."' ";
                                ...
                                $_sql9    = "SELECT kasift.id,kasift.jmlks,kasift.bsr from kasift where kasift.id='$baris[0]'";
                                ...
                                $_sql10   = "select sum(presensi.lembur) from presensi where presensi.id='$baris[0]' and presensi.tgl like '%".$tahun."-".$bulan."%' and presensi.jns='HB' group by presensi.id";
                                $hasil10  = bukaquery($_sql10);
                                $baris10  = mysqli_fetch_array($hasil10);
                                $hb       = isset($baris10[0])?$baris10[0]:0;

                                $_sql11   = "select sum(presensi.lembur) from presensi where presensi.id='$baris[0]' and presensi.tgl like '%".$tahun."-".$bulan."%' and presensi.jns='HR' group by presensi.id";
                                ...
                                $_sql17       = "SELECT set_jgmlm.tnj from set_jgmlm ";
                                $hasil17      = bukaquery($_sql17);
                                $baris17      = mysqli_fetch_array($hasil17);
                                $tmbhjgmlm    = $sisamlm*$baris17[0];
                                $ttltmbhjgmlm = $ttltmbhjgmlm+$tmbhjgmlm;

                                $_sql18       = "SELECT set_jgtambah.tnj from set_jgtambah where set_jgtambah.pendidikan='".$baris["pendidikan"]."'";
                                ...
                                $_sql19       = "SELECT set_hadir.tnj from set_hadir ";
                                ...
                                $_sqlpassum  = "select sum(jumpasien.jml) from jumpasien where jumpasien.thn='".$tahun."' and jumpasien.bln='".$bulanindex."'";
                                $hasilpassum = bukaquery($_sqlpassum);
                                $barispassum = mysqli_fetch_array($hasilpassum);
                                $indexpassum = $barispassum[0];
                                $indexpas    = 0;
                                if(!empty ($indexpassum)){
                                    $_sqlpas  = "select (jumpasien.jml/$indexpassum)*100 from jumpasien where jumpasien.id='$baris[0]' and jumpasien.thn='".$tahun."' and jumpasien.bln='".$bulanindex."'";
                                ...
                                $_sql24      = "select sum(tindakan.jm) from tindakan where tindakan.id='$baris[0]' and tindakan.tgl like '%".$tahun."-".$bulan."%' group by tindakan.id";
                                $hasil24     = bukaquery($_sql24);
                                $baris24     = mysqli_fetch_array($hasil24);
                                $jm          = (isset($baris24[0])?$baris24[0]:0);
                                $ttljm       = $ttljm+$jm;

                                $_sql26      = "select sum(jasa_lain.bsr_jasa) from jasa_lain where jasa_lain.id='$baris[0]' and jasa_lain.thn='".$tahun."' and jasa_lain.bln='".$bulanindex."' group by jasa_lain.id";
                                $hasil26     = bukaquery($_sql26);
                                $baris26     = mysqli_fetch_array($hasil26);

                                $_sql27      = "select (pembagian_akte.persen/100)*set_akte.bagian_kry from pembagian_akte,set_akte where pembagian_akte.id='$baris[0]'and set_akte.tahun='".$tahun."' and set_akte.bulan='".$bulanindex."'";
                                $hasil27     = bukaquery($_sql27);
                                $baris27     = mysqli_fetch_array($hasil27);

                                $_sql28      = "select (pembagian_resume.persen/100)*set_resume.pendapatan_resume from pembagian_resume,set_resume  where pembagian_resume.id='$baris[0]' and set_resume.tahun='".$tahun."' and set_resume.bulan='".$bulanindex."'";
                                $hasil28     = bukaquery($_sql28);
                                $baris28     = mysqli_fetch_array($hasil28);

                                $_sql29      = "select (pembagian_tuslah.persen/100)*set_tuslah.pendapatan_tuslah from pembagian_tuslah,set_tuslah  where pembagian_tuslah.id='$baris[0]' and set_tuslah.tahun='".$tahun."' and set_tuslah.bulan='".$bulanindex."'";
