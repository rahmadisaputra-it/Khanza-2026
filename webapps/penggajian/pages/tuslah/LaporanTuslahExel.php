<?php
    ...
    header("Content-Disposition: attachment; filename=LaporanTuslah.xls");
...
<html>
	...
	&nbsp;Pendapatan Tuslah :
        ...
        <?php
            ...
            $_sql 	= "SELECT pembagian_tuslah.id,pegawai.nama,pembagian_tuslah.persen FROM pembagian_tuslah inner join pegawai on pembagian_tuslah.id=pegawai.id where pegawai.nama like '%".$keyword."%' ORDER BY persen desc";
            $hasil	= bukaquery($_sql);
            $jumlah	= mysqli_num_rows($hasil);
            $ttl	= 0;
            $prosen	= 0;
