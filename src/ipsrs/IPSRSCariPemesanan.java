...
public class IPSRSCariPemesanan extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public IPSRSCariPemesanan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        label10.setPreferredSize(new java.awt.Dimension(61, 23));
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            if(ceksukses==false){
                ...
                Valid.MyReportqry("rptPemesananIPSRS.jasper","report","::[ Transaksi Penerimaan Barang Non Medis ]::","select * from temporary where temporary.temp37='"+akses.getalamatip()+"' order by temporary.no",param);
            }else{
    ...
    private void btnAmbilPhotoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilPhotoActionPerformed
        if(tbDokter.getSelectedRow()!= -1){
            if(!tbDokter.getValueAt(tbDokter.getSelectedRow(),8).toString().trim().equals("")){
                ...
                Valid.panggilUrl("penerimaanlogistik/login.php?act=login&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB()+"&nofaktur="+tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString());
                this.setCursor(Cursor.getDefaultCursor());
            }
        }
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            IPSRSCariPemesanan dialog = new IPSRSCariPemesanan(new javax.swing.JFrame(), true);
       ...
       Valid.tabelKosong(tabMode);
        try{   
            ps=koneksi.prepareStatement(
                    "select ipsrspemesanan.tgl_pesan,ipsrspemesanan.no_faktur,ipsrspemesanan.kode_suplier,ipsrssuplier.nama_suplier,ipsrspemesanan.nip,petugas.nama,"+
                    "ipsrspemesanan.tgl_faktur,ipsrspemesanan.tgl_tempo,ipsrspemesanan.status,ipsrspemesanan.total2,ipsrspemesanan.ppn,ipsrspemesanan.meterai,ipsrspemesanan.tagihan,"+
                    "ipsrspemesanan.no_order from ipsrspemesanan inner join ipsrssuplier on ipsrspemesanan.kode_suplier=ipsrssuplier.kode_suplier "+
                    "inner join petugas on ipsrspemesanan.nip=petugas.nip inner join ipsrsdetailpesan on ipsrspemesanan.no_faktur=ipsrsdetailpesan.no_faktur "+
                    "inner join ipsrsbarang on ipsrsdetailpesan.kode_brng=ipsrsbarang.kode_brng inner join kodesatuan on ipsrsdetailpesan.kode_sat=kodesatuan.kode_sat "+
                    "inner join ipsrsjenisbarang on ipsrsbarang.jenis=ipsrsjenisbarang.kd_jenis where ipsrspemesanan.tgl_pesan between ? and ? "+
                    (NoFaktur.getText().trim().equals("")?"":"and ipsrspemesanan.no_faktur=? ")+(nmsup.getText().trim().equals("")?"":"and ipsrspemesanan.kode_suplier=? ")+
                    (nmptg.getText().trim().equals("")?"":"and ipsrspemesanan.nip=? ")+(nmjenis.getText().trim().equals("")?"":"and ipsrsbarang.jenis=? ")+
                    (nmbar.getText().trim().equals("")?"":"and ipsrsdetailpesan.kode_brng=? ")+(TCari.getText().trim().equals("")?"":"and (ipsrspemesanan.no_faktur like ? or "+
                    "ipsrspemesanan.kode_suplier like ? or ipsrssuplier.nama_suplier like ? or ipsrspemesanan.nip like ? or petugas.nama like ? or ipsrsdetailpesan.kode_brng like ? or "+
                    "ipsrsbarang.nama_brng like ? or ipsrsdetailpesan.kode_sat like ? or ipsrspemesanan.no_order like ? or ipsrsjenisbarang.nm_jenis like ?) ")+
            ...
            try {
                ...
                while(rs.next()){
                    ...
                    ps2=koneksi.prepareStatement(
                        "select ipsrsdetailpesan.kode_brng,ipsrsbarang.nama_brng,ipsrsdetailpesan.kode_sat,kodesatuan.satuan,ipsrsdetailpesan.jumlah,ipsrsdetailpesan.harga, "+
                        "ipsrsdetailpesan.subtotal,ipsrsdetailpesan.dis,ipsrsdetailpesan.besardis,ipsrsdetailpesan.total from ipsrsdetailpesan "+
                        "inner join ipsrsbarang on ipsrsdetailpesan.kode_brng=ipsrsbarang.kode_brng inner join kodesatuan on ipsrsdetailpesan.kode_sat=kodesatuan.kode_sat "+
                        "inner join ipsrsjenisbarang on ipsrsbarang.jenis=ipsrsjenisbarang.kd_jenis where ipsrsdetailpesan.no_faktur=? "+
                        (nmjenis.getText().trim().equals("")?"":"and ipsrsbarang.jenis=? ")+(nmbar.getText().trim().equals("")?"":"and ipsrsdetailpesan.kode_brng=? ")+
                        (TCari.getText().trim().equals("")?"":"and (ipsrsdetailpesan.kode_brng like ? or ipsrsbarang.nama_brng like ? or ipsrsdetailpesan.kode_sat like ? or "+
                        "ipsrsjenisbarang.nm_jenis like ?) ")+"order by ipsrsdetailpesan.kode_brng");
                    try {
                        i=2;
