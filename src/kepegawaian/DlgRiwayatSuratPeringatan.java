    ...
    private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCariActionPerformed
        try {
            if(engine.getLocation().contains("ListRiwayatSuratPeringatan")){
                runBackground(() ->loadURL("http://" +koneksiDB.HOSTHYBRIDWEB()+":"+prop.getProperty("PORTWEB")+"/"+prop.getProperty("HYBRIDWEB")+"/"+"penggajian/index.php?act=ListRiwayatSuratPeringatan&action=LIHAT&keyword="+TCari.getText().replaceAll(" ","_")));
            }                
        } catch (Exception ex) {
            System.out.println("Notifikasi : "+ex);
        }        
