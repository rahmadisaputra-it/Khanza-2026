...
public class DapurSirkulasiBarang2 extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DapurSirkulasiBarang2(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();

        tabMode=new DefaultTableModel(null,new Object[]{
                "Kode Barang","Nama Barang","Satuan","Tgl.Opname","Stok Awal","Stok Awal(Rp)","Pengadaan","Pengadaan(Rp)",
                "Penerimaan","Penerimaan(Rp)","Stok Keluar","Stok Keluar(Rp)","Hibah","Hibah(Rp)","Stok Akhir","Stok Akhir(Rp)"
            }){
...
*/
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        if(tabMode.getRowCount()==0){
            ...
            for(int i=0;i<row;i++){  
                Sequel.menyimpan("temporary","'"+i+"',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'','','','','','','','','','','','','','','','','','','','','"+akses.getalamatip()+"'",16,new String[]{
            ...
            Valid.MyReportqry("rptSirkulasiDapur2.jasper","report","::[ Sirkulasi Barang Dapur Kering & Basah ]::","select * from temporary where temporary.temp37='"+akses.getalamatip()+"' order by temporary.no",param);
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DapurSirkulasiBarang2 dialog = new DapurSirkulasiBarang2(new javax.swing.JFrame(), true);
    ...
    private void prosesCari() {
       ...
       try{   
            ...
            try {
                ttltotalbeli=0;ttltotalpesan=0;ttltotalkeluar=0;ttltotalstokawal=0;ttltotalstokakhir=0;ttltotalhibah=0;
                ...
                while(rs.next()){
                    ...
                    stokakhir=0;tglopname="0000-00-00";totalstokawal=0;totalstokakhir=0;
                    jumlahhibah=0;totalhibah=0;
                    tglopname=Sequel.cariIsi("select dapuropname.tanggal from dapuropname where dapuropname.kode_brng='"+rs.getString(1)+"' and dapuropname.tanggal between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+"' order by dapuropname.tanggal asc limit 1");
                    ...
                    ps2=koneksi.prepareStatement("select sum(dapurdetailbeli.jumlah), sum(dapurdetailbeli.subtotal) "+
                        ...
                        " where dapurdetailbeli.kode_brng=? and dapurpembelian.tgl_beli "+
                        " between ? and ? ");
                    try {
                    ...
                    ps2=koneksi.prepareStatement("select sum(dapurdetailpesan.jumlah), sum(dapurdetailpesan.subtotal) "+
                        ...
                        " where dapurdetailpesan.kode_brng=? and dapurpemesanan.tgl_pesan "+
                        " between ? and ? ");
                    try {
                        ps2.setString(1,rs.getString(1));
                        ps2.setString(2,tglopname);
                    ...
                    ps2=koneksi.prepareStatement("select sum(dapurdetailpengeluaran.jumlah), sum(dapurdetailpengeluaran.total) "+
                        ...
                        " where dapurdetailpengeluaran.kode_brng=? and "+
                        " dapurpengeluaran.tanggal between ? and ?");
                    try {
                        ps2.setString(1,rs.getString(1));
                        ps2.setString(2,tglopname);
                        ps2.setString(3,Valid.SetTgl(Tgl2.getSelectedItem()+""));
                    ...
                    ps2=koneksi.prepareStatement("select sum(dapur_detail_hibah.jumlah), sum(dapur_detail_hibah.subtotalhibah) "+
                        ...
                        " where dapur_detail_hibah.kode_brng=? and dapur_hibah.tgl_hibah "+
                        " between ? and ? ");
                    try {
                        ps2.setString(1,rs.getString(1));
                    ...
                    if((jumlahbeli>0)||(jumlahpesan>0)||(jumlahkeluar>0)||(stok>0)||(jumlahhibah>0)){
                        if(stokawal<=0){
                            stokawal=stok-jumlahbeli-jumlahpesan+jumlahkeluar-jumlahhibah;
                            stokakhir=stok;
                            totalstokawal=totalstok-totalbeli-totalpesan+totalkeluar-totalhibah;
                            totalstokakhir=totalstok;
                        }else{
                            stokakhir=stokawal+jumlahbeli+jumlahpesan-jumlahkeluar+jumlahhibah;
                            totalstokakhir=totalstokawal+totalbeli+totalpesan-totalkeluar+totalhibah;
                        }
                ...
                tabMode.addRow(new Object[]{"<>>","Total :","","","",Valid.SetAngka(ttltotalstokawal),"",
                   ...
                   "",Valid.SetAngka(ttltotalhibah),"",Valid.SetAngka(ttltotalstokakhir)
                }); 
            } catch (Exception e) {
