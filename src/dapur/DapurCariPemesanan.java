...
public class DapurCariPemesanan extends javax.swing.JDialog {
    ...
    private PreparedStatement ps,ps2,pscaripesan,psdapurdetailpesan;
    ...
    private int i=0;
    public String Penerimaan_Dapur="",PPN_Masukan="",Kontra_Penerimaan_Dapur="";

    /** Creates new form DlgProgramStudi
     * @param parent
     * @param modal */
    public DapurCariPemesanan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            if(ceksukses==false){
                ...
                Valid.MyReportqry("rptPemesananDapur.jasper","report","::[ Transaksi Penerimaan Barang Dapur ]::","select * from temporary where temporary.temp37='"+akses.getalamatip()+"' order by temporary.no",param);
            }else{
    ...
    private void btnAmbilPhotoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilPhotoActionPerformed
        if(tbDokter.getSelectedRow()!= -1){
            if(!tbDokter.getValueAt(tbDokter.getSelectedRow(),8).toString().trim().equals("")){
                ...
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                Sequel.meghapus("bukti_pemesanan_dapur", "no_faktur",tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString());
                Sequel.menyimpan2("bukti_pemesanan_dapur","?,''",1,new String[]{tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString()});
                Valid.panggilUrl("penerimaandapur/login.php?act=login&usere="+koneksiDB.USERHYBRIDWEB()+"&passwordte="+koneksiDB.PASHYBRIDWEB()+"&nofaktur="+tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString());
                this.setCursor(Cursor.getDefaultCursor());
            }
        }
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DapurCariPemesanan dialog = new DapurCariPemesanan(new javax.swing.JFrame(), true);
       ...
       Valid.tabelKosong(tabMode);
        try{   
            ps=koneksi.prepareStatement(
                    ...
                    "dapurpemesanan.nip,petugas.nama,dapurpemesanan.tgl_faktur, "+
                    "dapurpemesanan.tgl_tempo,dapurpemesanan.status,dapurpemesanan.total2,dapurpemesanan.ppn,"+
                    "dapurpemesanan.meterai,dapurpemesanan.tagihan,dapurpemesanan.no_order "+
                    " from dapurpemesanan inner join dapursuplier on dapurpemesanan.kode_suplier=dapursuplier.kode_suplier "+
                    " inner join petugas on dapurpemesanan.nip=petugas.nip "+
                    " inner join dapurdetailpesan on dapurpemesanan.no_faktur=dapurdetailpesan.no_faktur "+
                    " inner join dapurbarang on dapurdetailpesan.kode_brng=dapurbarang.kode_brng "+
                    " inner join kodesatuan on dapurdetailpesan.kode_sat=kodesatuan.kode_sat "+
                    " where dapurpemesanan.tgl_pesan between ? and ? and dapurpemesanan.no_faktur like ? and dapursuplier.nama_suplier like ? and petugas.nama like ?  and dapurbarang.jenis like ? and dapurbarang.nama_brng like ? "+
                    (TCari.getText().trim().equals("")?"":" and (dapurpemesanan.no_faktur like ? or dapurpemesanan.kode_suplier like ? or dapursuplier.nama_suplier like ? or dapurpemesanan.nip like ? or petugas.nama like ? or "+
                    "dapurdetailpesan.kode_brng like ? or dapurbarang.nama_brng like ? or dapurdetailpesan.kode_sat like ? or dapurpemesanan.no_order like ? or dapurbarang.jenis like ?) ")+
                    " group by dapurpemesanan.no_faktur order by dapurpemesanan.tgl_pesan,dapurpemesanan.no_faktur ");
            try {
                ...
                while(rs.next()){
                    ...
                    ps2=koneksi.prepareStatement("select dapurdetailpesan.kode_brng,dapurbarang.nama_brng, "+
                        ...
                        "from dapurdetailpesan inner join dapurbarang on dapurdetailpesan.kode_brng=dapurbarang.kode_brng "+
                        "inner join kodesatuan on dapurdetailpesan.kode_sat=kodesatuan.kode_sat where "+
                        " dapurdetailpesan.no_faktur=? and dapurbarang.jenis like ? and dapurbarang.nama_brng like ? "+
                        (TCari.getText().trim().equals("")?"":" and (dapurdetailpesan.kode_brng like ? or dapurbarang.nama_brng like ? "+
                        "or dapurdetailpesan.kode_sat like ? or dapurbarang.jenis like ?) ")+"order by dapurdetailpesan.kode_brng  ");
                    try {
    ...
    public void isCek(){
        BtnPrint.setEnabled(akses.getdapur_pemesanan());
                ...
                try {
                    ...
                    if(rs.next()){
                        if(rs.getString("photo").equals("")||rs.getString("photo").equals("-")){
                            ...
                            LoadHTML.setText("<html><body><center><img src='http://"+koneksiDB.HOSTHYBRIDWEB()+":"+koneksiDB.PORTWEB()+"/"+koneksiDB.HYBRIDWEB()+"/penerimaandapur/"+rs.getString("photo")+"' alt='photo' width='"+(internalFrame1.getWidth()-335)+"' height='"+(internalFrame1.getHeight()-265)+"'/></center></body></html>");
                        }  
                    }else{
                        LoadHTML.setText("<html><body><center><br><br><font face='tahoma' size='2' color='#434343'>Kosong</font></center></body></html>");
                    }
