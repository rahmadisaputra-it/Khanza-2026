                    ...
                    if(koderekening.equals("")){
                        ...
                        if(koderekening.equals(Host_to_Host_Bank_Mandiri)){
                            ...
                            ps=koneksi.prepareStatement(
                                    "select tokosuplier.nama_suplier,tokosuplier.kota,tokosuplier.nama_bank,tokosuplier.rekening "+
                                    "from tokopemesanan inner join tokosuplier on tokopemesanan.kode_suplier=tokosuplier.kode_suplier "+
                                    "where tokopemesanan.no_faktur=?");
                            try{
                            ...
                            if(sukses==true){
                                if((sisahutang<=Double.parseDouble(BesarBayar.getText()))||(sisahutang<=-Double.parseDouble(BesarBayar.getText()))){
                                    Sequel.mengedit("tokopemesanan","no_faktur=?","status='Sudah Dibayar'",1,new String[]{NoFaktur.getText()});
                                }else{
                                    Sequel.mengedit("tokopemesanan","no_faktur=?","status='Belum Lunas'",1,new String[]{NoFaktur.getText()});
                                }
        ...
        try {
            ...
            if(Sequel.queryu2tf("delete from toko_bayar_pemesanan where tgl_bayar=? and no_faktur=? and "+
                ...
                if(Double.parseDouble(tbKamar.getValueAt(tbKamar.getSelectedRow(),8).toString())==Double.parseDouble(BesarBayar.getText())){
                    Sequel.mengedit("tokopemesanan","no_faktur=?","status='Belum Dibayar'",1,new String[]{NoFaktur.getText()});
                }else{
                    Sequel.mengedit("tokopemesanan","no_faktur=?","status='Belum Lunas'",1,new String[]{NoFaktur.getText()});
                }
                ...
                if(sukses==true){
                    ...
                    if(Sequel.menyimpantf2("toko_bayar_pemesanan","?,?,?,?,?,?,?","data", 7,new String[]{
                        ...
                        if(Sequel.menyimpantf("pembayaran_pihak_ke3_bankmandiri","?,now(),?,?,?,?,?,?,?,?,?,?,?","No.Bukti", 12,new String[]{
                                NoBukti.getText(),norekening,NoRekening.getText(),RekeningAtasNama.getText(),KotaAtasNamaRekening.getText(),BesarBayar.getText(),NoFaktur.getText(),KodeMetode.getText(),KodeBank.getText(),KodeTransaksi.getText(),"Bayar Pesan Toko","Baru"
                            })==false){
                            sukses=false;
                        }
                    }else{
