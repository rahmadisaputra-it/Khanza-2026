 ...
 * @author perpustakaan
 */
public final class KeuanganHutangDapurBelumLunas extends javax.swing.JDialog {
    ...
    private int row=0,i;
    private String koderekening="",tanggaldatang="",tanggaltempo="",Bayar_Pemesanan_Dapur=Sequel.cariIsi("select set_akun2.Bayar_Pemesanan_Dapur from set_akun2"),Host_to_Host_Bank_Mandiri="",Akun_Biaya_Mandiri="",kodemcm="",norekening="";
     ...
     * @param parent
     * @param modal */
    public KeuanganHutangDapurBelumLunas(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void initComponents() {
        ...
        KdPetugas.setBounds(428, 10, 100, 23);

        NmPetugas.setEditable(false);
        NmPetugas.setName("NmPetugas"); // NOI18N
        NmPetugas.setPreferredSize(new java.awt.Dimension(207, 23));
        panelisi4.add(NmPetugas);
        NmPetugas.setBounds(530, 10, 190, 23);
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            Valid.MyReportqry("rptHutangNonMedisBelumLunas.jasper","report","::[ Data Hutang Barang Dapur ]::","select * from temporary where temporary.temp37='"+akses.getalamatip()+"' order by temporary.no",param);
        }
...
private void BtnCariKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnCariKeyPressed
            ...
            if(koderekening.equals("")){
                ...
                if(koderekening.equals(Host_to_Host_Bank_Mandiri)){
                    if(KodeSuplier.getText().trim().equals("")||NamaSuplier.getText().trim().equals("")){
                        ...
                        try {
                            myObj = new FileReader("./cache/suplierdapur.iyem");
                            root = mapper.readTree(myObj);
                            response = root.path("suplierdapur");
                            if(response.isArray()){
                                for(JsonNode list:response){
                    ...
                    for(int i=0;i<row;i++){  
                        if(tabMode.getValueAt(i,0).toString().equals("true")){
                            if(Double.parseDouble(tbBangsal.getValueAt(i,10).toString())>0){
                                if(Sequel.menyimpantf("bayar_pemesanan_dapur","?,?,?,?,?,?,?","Data", 7,new String[]{
                                    Valid.SetTgl(TglBayar.getSelectedItem()+""),tabMode.getValueAt(i,1).toString(),KdPetugas.getText(),
                                    ...
                                    if((Double.parseDouble(tabMode.getValueAt(i,9).toString())<=0)||(Double.parseDouble(tabMode.getValueAt(i,9).toString())<=-0)){
                                        Sequel.mengedit("dapurpemesanan","no_faktur='"+tabMode.getValueAt(i,1).toString()+"'","status='Sudah Dibayar'");
                                    }else{
                                    ...
                                    if(Sequel.menyimpantf2("tampjurnal","?,?,?,?","Rekening",4,new String[]{
                                        Bayar_Pemesanan_Dapur,"HUTANG BARANG DAPUR",tabMode.getValueAt(i,10).toString(),"0"
                                    })==false){
                                        sukses=false;
                                    }                       
                                    if(Sequel.menyimpantf2("tampjurnal","?,?,?,?","Rekening",4,new String[]{
                    ...
                    if(sukses==true){
                        if(!notagihan.equals("")){
                            Sequel.queryu("update dapur_titip_faktur set status='Dibayar' where no_tagihan=?",notagihan);
                            notagihan="";
                        }
                        Sequel.Commit();
                    }else{
    ...
    private void KeteranganKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_KeteranganKeyPressed
        Valid.pindah(evt,KdPetugas,BtnBayar);
                        ...
                        if(Double.parseDouble(tbBangsal.getValueAt(i,10).toString())>0){
                            ...
                            if(Sequel.menyimpantf("bayar_pemesanan_dapur","?,?,?,?,?,?,?","Data", 7,new String[]{
                                ...
                                if(Sequel.menyimpantf("pembayaran_pihak_ke3_bankmandiri","?,now(),?,?,?,?,?,?,?,?,?,?,?","No.Bukti", 12,new String[]{
                                        NoBukti.getText(),norekening,NoRekening.getText(),RekeningAtasNama.getText(),KotaAtasNamaRekening.getText(),tabMode.getValueAt(i,10).toString(),tabMode.getValueAt(i,1).toString(),KodeMetode.getText(),KodeBank.getText(),KodeTransaksi.getText(),"Bayar Pesan Dapur","Baru"
                                    })==false){
                                    sukses=false;
                                }else{
                                    Sequel.queryu("delete from tampjurnal");
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            KeuanganHutangDapurBelumLunas dialog = new KeuanganHutangDapurBelumLunas(new javax.swing.JFrame(), true);
    ...
    private void tampil(){
        ...
        try{
            ...
            ps=koneksi.prepareStatement(
                    ...
                    "dapursuplier.nama_bank,dapursuplier.rekening from dapurpemesanan inner join dapursuplier on dapurpemesanan.kode_suplier=dapursuplier.kode_suplier "+
                    "inner join petugas on dapurpemesanan.nip=petugas.nip where "+
    ...
    private void panggilPhoto() {
        if(FormPhoto.isVisible()==true){
            try {
                ps=koneksi.prepareStatement("select photo from bukti_pemesanan_dapur where no_faktur=?");
                try {
    ...
    public void isCek(){
        BtnBayar.setEnabled(akses.getbayar_pesan_dapur());
        if(akses.getjml2()>=1){
    ...
    private void tampilTagihan(String notagihan){
        ...
        try{
            ps=koneksi.prepareStatement(
                    "select dapurpemesanan.no_faktur,dapurpemesanan.no_order,dapursuplier.nama_suplier,dapurpemesanan.kode_suplier, "+
                    "petugas.nama,dapurpemesanan.tgl_tempo,dapurpemesanan.tgl_pesan,dapurpemesanan.tgl_faktur,dapurpemesanan.tagihan,"+
                    "(SELECT ifnull(SUM(bayar_pemesanan_dapur.besar_bayar),0) FROM bayar_pemesanan_dapur where bayar_pemesanan_dapur.no_faktur=dapurpemesanan.no_faktur) as bayar, "+
                    "dapursuplier.nama_bank,dapursuplier.rekening from dapurpemesanan "+
                    ...
                    "inner join dapur_detail_titip_faktur on dapur_detail_titip_faktur.no_faktur=dapurpemesanan.no_faktur "+
                    "where dapurpemesanan.status<>'Sudah Dibayar' and dapur_detail_titip_faktur.no_tagihan=?");
            try {
