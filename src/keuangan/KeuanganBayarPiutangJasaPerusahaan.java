 ...
 * @author dosen
 */
public final class KeuanganBayarPiutangJasaPerusahaan extends javax.swing.JDialog {
    ...
    private String koderekening="",Piutang_Jasa_Perusahaan=Sequel.cariIsi("select set_akun2.Piutang_Jasa_Perusahaan from set_akun2");
     ...
     * @param parent
     * @param modal */
    public KeuanganBayarPiutangJasaPerusahaan(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
            "Tgl.Bayar","Kode","Instansi/Perusahaan","Cicilan(Rp)","Keterangan","No.Piutang","Kode Akun","Akun Bayar"}){
        ...
        NoPiutang.setDocument(new batasInput((byte)17).getKata(NoPiutang));
    ...
    private void initComponents() {
        ...
        FormInput.add(NoPiutang);
        NoPiutang.setBounds(79, 10, 180, 23);
        ...
        FormInput.add(KdPerusahaan);
        KdPerusahaan.setBounds(384, 10, 90, 23);
        ...
        NmPerusahaan.setBounds(476, 10, 204, 23);
        ...
        BtnPerusahaan.setBounds(682, 10, 28, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoPiutang.getText().trim().equals("")){
            Valid.textKosong(NoPiutang,"No.Tagihan/No.Rawat");
            ...
            Valid.textKosong(KdPerusahaan,"Member");
        }else if(Keterangan.getText().trim().equals("")){
            Valid.textKosong(Keterangan,"Keterangan");
        }else if(AkunBayar.getSelectedItem()==null){
            Valid.textKosong(AkunBayar,"Akun Bayar");
        }else{         
            if(Sequel.cariInteger("select count(piutang_jasa_perusahaan.no_piutang) from piutang_jasa_perusahaan where piutang_jasa_perusahaan.no_piutang=? and piutang_jasa_perusahaan.kode_perusahaan=?",NoPiutang.getText(),KdPerusahaan.getText())>0){
                sisapiutang=(Sequel.cariIsiAngka("SELECT piutang_jasa_perusahaan.sisapiutang FROM piutang_jasa_perusahaan where piutang_jasa_perusahaan.no_piutang=?",NoPiutang.getText())-Double.parseDouble(Cicilan.getText()));
                ...
                if(Sequel.menyimpantf("bayar_piutang_jasa_perusahaan","?,?,?,?,?,?,?","Pembayaran",7,new String[]{
                        ...
                        Keterangan.getText(),NoPiutang.getText(),koderekening,AkunBayar.getSelectedItem().toString()
                    })==true){
                        if(sisapiutang<=1){
                            Sequel.mengedit("piutang_jasa_perusahaan","no_piutang='"+NoPiutang.getText()+"'","status='Sudah Lunas'");
                        }   
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if(tbKamar.getSelectedRow()>-1){
            ...
            if(Sequel.queryu2tf("delete from bayar_piutang_jasa_perusahaan where tgl_bayar=? and kode_perusahaan=? and no_piutang=? and kd_rek=? and nama_bayar=?", 5,new String[]{
                ...
                Sequel.queryu("delete from tampjurnal");                    
                if(Sequel.menyimpantf2("tampjurnal","'"+Piutang_Jasa_Perusahaan+"','PIUTANG JASA PERUSAHAAN','"+tbKamar.getValueAt(tbKamar.getSelectedRow(),3).toString()+"','0'","Rekening")==false){
                    sukses=false;
                }   
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            Valid.MyReportqry("rptBayarPiutangJasaPerusahaan.jasper","report","::[ Bayar Piutang Jasa Perusahaan ]::",
                ...
                "bayar_piutang_jasa_perusahaan.tgl_bayar between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+"' and bayar_piutang_jasa_perusahaan.no_piutang like '%"+TCari.getText()+"%' or "+
                "bayar_piutang_jasa_perusahaan.tgl_bayar between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+"' and bayar_piutang_jasa_perusahaan.kode_perusahaan like '%"+TCari.getText()+"%' or "+
                "bayar_piutang_jasa_perusahaan.tgl_bayar between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+"' and perusahaan_pasien.nama_perusahaan like '%"+TCari.getText()+"%' or "+
                "bayar_piutang_jasa_perusahaan.tgl_bayar between '"+Valid.SetTgl(Tgl1.getSelectedItem()+"")+"' and '"+Valid.SetTgl(Tgl2.getSelectedItem()+"")+"' and bayar_piutang_jasa_perusahaan.tgl_bayar like '%"+TCari.getText()+"%' "+
...
private void BtnPerusahaanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPerusahaanActionPerformed
    ...
    private void BtnCari1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCari1ActionPerformed
        if(NoPiutang.getText().trim().equals("")){
            Valid.textKosong(NoPiutang,"No.Nota");
        }else{
            if(Sequel.cariInteger("select count(piutang_jasa_perusahaan.no_piutang) from piutang_jasa_perusahaan where piutang_jasa_perusahaan.no_piutang=?",NoPiutang.getText())>0){
                KdPerusahaan.setText(Sequel.cariIsi("select piutang_jasa_perusahaan.kode_perusahaan from piutang_jasa_perusahaan where piutang_jasa_perusahaan.no_piutang=?",NoPiutang.getText()));
                NmPerusahaan.setText(Sequel.cariIsi("select perusahaan_pasien.nama_perusahaan from perusahaan_pasien where perusahaan_pasien.kode_perusahaan=?",KdPerusahaan.getText()));
                sisapiutang=Sequel.cariIsiAngka("SELECT piutang_jasa_perusahaan.sisapiutang FROM piutang_jasa_perusahaan where piutang_jasa_perusahaan.no_piutang=?",NoPiutang.getText());
                Sisa.setText(Valid.SetAngka(Valid.roundUp(sisapiutang,100)));
            }else{
                JOptionPane.showMessageDialog(null,"No.Piutang tidak ditemukan......!!!");
            }
        }
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            KeuanganBayarPiutangJasaPerusahaan dialog = new KeuanganBayarPiutangJasaPerusahaan(new javax.swing.JFrame(), true);
    ...
    private void tampil() {
        ...
        try{    
            ps=koneksi.prepareStatement(
                    ...
                    "inner join perusahaan_pasien on bayar_piutang_jasa_perusahaan.kode_perusahaan=perusahaan_pasien.kode_perusahaan where "+
                    "bayar_piutang_jasa_perusahaan.tgl_bayar between ? and ? "+(TCari.getText().trim().equals("")?"":"and (bayar_piutang_jasa_perusahaan.no_piutang like ? or bayar_piutang_jasa_perusahaan.kode_perusahaan like ? or "+
                    "perusahaan_pasien.nama_perusahaan like ?) ")+"order by bayar_piutang_jasa_perusahaan.tgl_bayar,bayar_piutang_jasa_perusahaan.kode_perusahaan");
            try {
    ...
    public void setData(String nonota,String kodepeminjam,String namapeminjam){
        NoPiutang.setText(nonota);
        KdPerusahaan.setText(kodepeminjam);
        NmPerusahaan.setText(namapeminjam);
    ...
    private void getData() {
        ...
        if(row!= -1){
            KdPerusahaan.setText(tbKamar.getValueAt(row,1).toString());
            NmPerusahaan.setText(tbKamar.getValueAt(row,2).toString());
            Cicilan.setText(tbKamar.getValueAt(row,3).toString());
            Keterangan.setText(tbKamar.getValueAt(row,4).toString());
            NoPiutang.setText(tbKamar.getValueAt(row,5).toString());
    ...
    public void isCek(){
        BtnSimpan.setEnabled(akses.getbayar_piutang_jasa_perusahaan());
        BtnHapus.setEnabled(akses.getbayar_piutang_jasa_perusahaan());
        BtnPrint.setEnabled(akses.getbayar_piutang_jasa_perusahaan());
    }
