        ...
        if($status == "batal"){
            ...
            if($interval<0){
                ...
                if($rsquerypasien = mysqli_fetch_array($querypasien)) {
                    ...
                    $insert     = Tambah4("reg_periksa","'$no_reg','$no_rawat','$tanggal',current_time(),'$kd_dokter','".cleankar(encrypt_decrypt($_SESSION["ses_pasien"],"d"))."','$kd_poli','".$rsquerypasien["namakeluarga"]."','".$rsquerypasien["alamatpj"]."','".$rsquerypasien["keluarga"]."','".getOne2("select registrasilama from poliklinik where kd_poli='$kd_poli'")."','Belum','Lama','Ralan','$kd_pj','$umur','$sttsumur','Belum Bayar','$statuspoli'");
                    if($insert){
